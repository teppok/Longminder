...
public class UserManagerService implements Serializable {
    ...
    /**
     * Takes the user name and password and calls the HttpServletRequest with these values (properly salted).
     * @param name
     * @param password
     * @param req
    ...
    public boolean login(String name, String password, HttpServletRequest req) {
    	System.out.println("Login: " + req.getRemoteUser() + " / " + req.isUserInRole("USER"));
    ...
    /**
     * Given an UserDTO instance with valid values, creates a new user and logs him
     * in with login(). Also sends a verification email.
     * 
     * UserDTO is valid if: email, password1, password2, first, last have non-empty strings as values.
     * 
     * @param ud
     * @param req
     * @return true if successful
    ...
    public boolean register(UserDTO ud, HttpServletRequest req) {
    	...
    	if (ud == null || ud.getEmail() == null || ud.getPassword1() == null || ud.getEmail().length() == 0) {
    		return false;
    	}
    	System.out.println("Registering " + ud.getEmail() + " " + ud.getPassword1());
    	...
    	if (ud.getLocale() == null) { ud.setLocale("en-US"); }
    ...
    /**
     * Given a temporary key, verifies the email of the user tied to this temporary key.
     * @param key
     * @return true if successful
     */
     ...
     * UserManager requires that if email or password is changed, old password must
     * be given, and if this fails, we fail.
     * 
     * If the modification was successful and email was modified, we log out the user
     * and log him back in.
     * 
     * @param modifyUser
     * @param req
...
//		boolean passwordCheck = um.verifyPassword(email, oldPassword);
		String oldLoginName = req.getRemoteUser();
		if (oldLoginName == null || !oldLoginName.equals(modifyUser.getEmail())) return false;
		...
		if (! oldLoginName.equals(modifyUser.getEmail())) {
	    	try {
	    		System.out.println("Logging out!" + modifyUser.getEmail() + " " + modifyUser.getReturnPassword());
				req.logout();
	...
	/**
	 * Fills the given UserDTO instance with data from database regarding the current user.
	 * @param req
	 * @param modifyUser
	 */
	...
	// TODO Don't actually send it now, just place a request to be performed on the EJB schedule thread.
