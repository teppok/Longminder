...
//		em.persist(a);
	...
	*/
	...
	public Alert findTrueAlert(String email, int id) {
		...
		List<Alert> alerts = em.createQuery("SELECT a FROM User u JOIN u.alerts a WHERE u.email = ?1 AND a.id = ?2", Alert.class).setParameter(1, email).setParameter(2, new Integer(id)).getResultList();
...
//   @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	...
	public List<AlertDTO> getAlerts(String email) {
		...
		List<Alert> listResult = em.createQuery("SELECT a FROM User u JOIN u.alerts a WHERE u.email = ?1 ORDER BY a.nextAlert DESC", Alert.class).setParameter(1, email).getResultList();
	...
	public List<AlertDTO> getAlertsForList(String email) {
		...
		List<Alert> listResult = em.createQuery("SELECT a FROM User u JOIN u.alerts a WHERE u.email = ?1 AND " +
		"((a.oneOff = true AND a.fired = false) OR (a.oneOff = false AND a.dismissed = false)) ORDER BY a.nextAlert ASC", Alert.class).setParameter(1, email).getResultList();
	...
	public List<AlertDTO> getAlertsForHistory(String email) {
		...
		List<Alert> listResult = em.createQuery("SELECT a FROM User u JOIN u.alerts a WHERE u.email = ?1 AND " +
				"(a.fired = true) ORDER BY a.nextAlert ASC", Alert.class).setParameter(1, email).getResultList();
